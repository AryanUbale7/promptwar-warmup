...
// Helper to generate dynamic high-quality mock data based on user input
const generateMockMealPlan = (inputs) => {
  const isINR = inputs.currency === 'INR';
  const symbol = isINR ? '₹' : '$';
  const num = parseInt(inputs.peopleCount, 10) || 1;
  const skill = inputs.skillLevel;
  const restrictions = inputs.dietaryRestrictions;
  const budgetVal = parseFloat(inputs.budget) || 100;
  
  // Determine vegetarian/vegan/gluten-free options
...
function App() {
  ...
  // Sanitize input to prevent injection issues or prompt escape
  const sanitizeInput = (text) => {
    if (!text) return '';
    return text
      .replace(/<[^>]*>?/gm, '') // Strip HTML tags
      .replace(/[{}[\]\\]/g, ' ') // Strip raw brackets that confuse JSON prompts
      .trim();
  };
